...
for i in range(iter_time):
    ...
    for j in range(len(lines)):
        ...
        if 'Noise correlation time' in lines[j]:
            lines[j] = 'Noise correlation time = 5' +' \n'
...
for i in range(iter_time):
    ...
    for j in range(len(lines)):
        if 'seed' in lines[j]:
            lines[j] = 'random seed = ' + str((i+1)*100) +' \n'
        if 'Noise correlation time' in lines[j]:
            lines[j] = 'Noise correlation time = 2' +' \n'
    f = open(S1_cin_name,'w')
    f.writelines(lines)
    f.close()
...
for i in range(iter_time):
    ...
    for j in range(len(lines)):
        if 'seed' in lines[j]:
            lines[j] = 'random seed = ' + str((i+1)*100) +' \n'
        if 'Noise correlation time' in lines[j]:
            lines[j] = 'Noise correlation time = 8' +' \n'
    f = open(S1_cin_name,'w')
    f.writelines(lines)
    f.close()
